...
class SHCPostProcFv(object):
    ...
    def __init__(self,compactFvFile,reCalcFv=False,**args):
        ...
        self.SHC_k0_x_smooth=None
        self.SHC_k0_x_smooth2=None
        self.SHC_k0_x_average=None
        self.SHC_k0_x_error=None
    ...
    def postProcess(self):
        ...
        self.SHC_k0_x_smooth=np.zeros(Nfreqs)
        self.SHC_k0_x_smooth2=np.zeros(Nfreqs)
        self.SHC_k0_x_average=np.zeros(Nfreqs)        
...
#        for k in range(0,2): # Start the iteration over chunks
            ...
            if NDOF%12!=0:
                ...
                SHC_k0=2*np.real(np.sum(Fs_k0*np.conj(vels_k0),axis=0))
                SHC_k0_x=2*np.real(np.sum(Fs_k0[0::3]*np.conj(vels_k0[0::3]),axis=0))
                
                # Normalize correctly
                SHC_k0/=(self.chunkSize*self.sampleTimestep)
                SHC_k0_x/=(self.chunkSize*self.sampleTimestep)

                # Change units             
                SHC_k0*=self.scaleFactor
                SHC_k0_x*=self.scaleFactor
            ...
            if not exitFlag: # If Nfreqs has changed, the running averaging cannot be performed
                ...
                self.SHC_k0_x_smooth=SHC_k0_x
                self.SHC_k0_x_smooth2=SHC_k0_x**2
                self.SHC_k0_x_average=SHC_k0_x_orig
                self.DoS_smooth=DoS
        ...
        if self.NChunks>1:
            ...
            samplevar=(self.NChunks/(self.NChunks-1.0))*(self.SHC_k0_x_smooth2-self.SHC_k0_x_smooth**2)
            self.SHC_k0_x_error=np.sqrt(samplevar)/np.sqrt(self.NChunks)
