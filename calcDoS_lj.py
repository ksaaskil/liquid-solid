...
import numpy as np
from DoSPostProc_k0 import DoSPostProc
...
def main(fileprefix):
    ...
    if 1:
        ...
        widthWin=0.25 # Width of the Daniell smoothing window
    else:
        dt_md=1e-15 # Timestep used in MD, affects the frequency grid
        widthWin=0.25e12
    # The force-velocity dump file from LAMMPS
    fileVels=fileprefix+'.vels_liquid.dat' 
    # fileVels=fileprefix+'.vels_interfaceL.dat' 
    ...
    with open(outputFolder+'/'+fileprefix+'_DoS.pckl','w') as f:
        pickle.dump(pP,f)
